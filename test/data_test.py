...
### DISCOVER
get_discover = '/discover/movie'


# 비디오
# get_video = f'movie/{movie_id}/videos'
# 디테일
# get_detail = f'movie/{movie_id}'


# django PJT에서 필요로 하는 field목록
fields = ['id','title', 'overview', 'release_date', 'poster_path', 'vote_average', 'popularity']
# fields = ['video_key']
...
for i in range(1, 50):
    ...
    response = requests.get(f'{base_url}{get_discover}', params={'api_key':API_KEY, 'page':i, 'language':'ko-KR', 'with_genres': 878} ).json()
    ...
    for res in response['results']:
        ...
        # print(movie_id)
        # print('-'*30)
        res2 = requests.get(f'{base_url}/movie/{movie_id}/videos', params={'api_key':API_KEY, 'language':'ko-KR'} ).json()
        # print(res2)
        # print(res2['results'][0]['key'])
        # fields2 = ['key']

        # for field in fields2:
        #     if res.get(field):
        #         movie_info[field] = res2[field]
        #     else:
        #         break
        # # for r in res2['results']:
        # #     if r['site'] == 'YouTube':
        # #         movie_info['key'] = r['key']
        # #     else:
        # #         break


        if res2['results']:
          if res2['results'][0]['site'] == 'YouTube':
            movie_info['key'] = res2['results'][0]['key']
        else:
          res3 = requests.get(f'{base_url}/movie/{movie_id}/videos', params={'api_key':API_KEY, 'language':'en-US'} ).json()
          if res3['results'] and res3['results'][0]['site'] == 'YouTube':
            movie_info['key'] = res3['results'][0]['key']
            cnt += 1
            print(movie_id)
        # else:
        #     break
...
fields = ['pk', 'id', 'title', 'overview', 'release_date', 'poster_path', 'vote_average', 'popularity', 'key']
